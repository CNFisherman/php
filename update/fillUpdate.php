...
try{
    ...
    if(!empty($_POST)){
        ...
        //定义合法字段数组
        $fields = array('fill_info','fill_answ','fill_level','fill_chapter','fill_score');
        ...
        //组合sql语句
        $sql = "update `fill` set $update_str where  `fill_id`=$fill_id";
    
        if($res = $pdo->query($sql)){
            header("Location: ../select/fillshow.php");
            die;
        }else{
            die('信息修改失败');
        }
        ...
        //编写SQL语句，查询相应ID的试题数据
        $sql = "select * from fill where `fill_id`=$fill_id";
