...
try{
    ...
    if(!empty($_POST)){
        ...
        //定义合法字段数组
        $fields = array('cal_info','cal_answ','cal_level','cal_chapter','cal_score');
        ...
        //组合sql语句
        $sql = "update `cal` set $update_str where  `cal_id`=$cal_id";
    
        if($res = $pdo->query($sql)){
            header("Location: ../select/calshow.php");
            die;
        }else{
            die('信息修改失败');
        }
        ...
        //编写SQL语句，查询相应ID的试题数据
        $sql = "select * from cal where `cal_id`=$cal_id";
