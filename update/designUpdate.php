...
try{
    ...
    if(!empty($_POST)){
        ...
        //定义合法字段数组
        $fields = array('design_info','design_answ','design_level','design_chapter','design_score');
        ...
        //组合sql语句
        $sql = "update `design` set $update_str where  `design_id`=$design_id";
    
        if($res = $pdo->query($sql)){
            header("Location: ../select/designshow.php");
            die;
        }else{
            die('信息修改失败');
        }
